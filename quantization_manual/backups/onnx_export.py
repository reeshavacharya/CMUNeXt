...
# 1. Start with a clean slate to avoid the hidden "disabled" defaults
config = {
    "quant_cfg": {
        # Wildcard for all weights: Per-channel (axis 0)
        "*weight_quantizer": {"num_bits": 8, "axis": 0, "enable": True},
        # Wildcard for all inputs/activations: Per-tensor (axis None)
        "*input_quantizer": {"num_bits": 8, "axis": None, "enable": True},
        # This catch-all ensures that even modules typically disabled
        # (like GELU or BatchNorm) are enabled if they contain a quantizer
        "default": {"num_bits": 8, "enable": True},
    },
    "algorithm": "max",
}
...
EXPORT_PATH = os.path.join(
    PROJECT_ROOT, "checkpoint", "quantized_manual", QUANTIZED_ONNX_NAME
)
