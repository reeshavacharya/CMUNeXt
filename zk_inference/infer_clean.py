...
def run_inference_once(case_name: str | None = None) -> None:
	"""Run inference on a single validation image and save metrics.

	If `case_name` is provided, use that specific case (base name without
	".png"). Otherwise, pick a random case from the validation list.
	"""
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description="Clean FP32 CMUNeXt inference")
	parser.add_argument(
		"--case",
		type=str,
		default=None,
		help="Case name (without .png). If omitted, pick a random validation case.",
	)
	args = parser.parse_args()
	run_inference_once(case_name=args.case)
